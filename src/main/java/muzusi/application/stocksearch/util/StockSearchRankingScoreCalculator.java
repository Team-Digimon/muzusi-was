...
public class StockSearchRankingScoreCalculator {
    ...
    private static final int WEIGHT = StockSearchMatchType.minScoreGap();

    /**
     * 매치 점수와 검색 빈도를 종합하여 최종 랭킹 점수를 계산하는 메서드
     *
     * <p> 검색 빈도({@code searchCount})가 그대로 반영되면 값이 커질수록 매치 점수를 압도해버리므로,
     *     검색 빈도가 최종 점수에 영향을 미치는 범위는 매치 점수의 최소 차이인 {@code WEIGHT}를 넘지못하도록
     *     포화 함수를 적용하여 계산한다.
     *
     * <p> 검색 빈도에 대한 포화 함수 결과값에 가중치 {@code WEIGHT}를 곱하여 검색 빈도가 최종 점수에 반영되는
     *     점수 범위를 [0, {@code WEIGHT})로 제한한다.
     *
